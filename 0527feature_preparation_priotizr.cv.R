...
crop_coords_dust <- read_csv("data/pland_pmp_bbau_lucas.csv")
...
cost <- st_read("/Users/lily/Library/CloudStorage/Box-Box/calvin/cost_node_dust.shp")
...
cost1 <- st_read("/Users/lily/Library/CloudStorage/Box-Box/calvin/cost_water_his.shp")
cost1 <- st_transform(cost1, crs = st_crs(cost_pu))
cost_pu1 <- st_join(cost_pu, cost1, join = st_nearest_feature)
cost2 <- st_read("/Users/lily/Library/CloudStorage/Box-Box/calvin/cost_water_bbau.shp")
cost2 <- st_transform(cost2, crs = st_crs(cost_pu))
cost_pu2 <- st_join(cost_pu1, cost2, join = st_nearest_feature)
pu_multi$cost_dust<-cost_pu2$cost_of_wa
pu_multi$cost_his<-cost_pu2$cost_of_wa.x
pu_multi$cost_bbau<-cost_pu2$cost_of_wa.y
...
sagbi <- st_transform(sagbi, crs = crs(land_cover_pmp))

# # Convert MultiPolygon to Polygon
sagbi1 <- st_cast(sagbi, "POLYGON")
# Check and repair geometries
sagbi_repaired <- st_make_valid(sagbi1)
# Simplify the repaired geometries
sagbi_simplified <- st_simplify(sagbi_repaired)
# 
crops_sagbi<-c("rice", 'pasture', 'alfalfa', 'grain', "perennial", "field_row", "barren")

sagbi_multipliers<-c(rice=2, alfalfa=2, pasture=5, perennial=3, grain=2, field_row=1, barren=4)


# Define a function to calculate the carbon sequestion potential
calculate_rchSuit <- function(cell) {
  
  # Check if points in land_cover_data are within the multipolygon cell
  is_within <- sapply(st_intersects(sagbi, cell), function(x) length(x) > 0)
  sagbi_value<-colMeans(st_drop_geometry(sagbi[is_within, "sagbi"]))
  
  # Extract the rice value from land_cover_data for the corresponding hexagon cell
  crop_areas <- land_cover_pmp[i,][, crops_sagbi]  # Assuming columns 6 to 14 contain the crop values
  ...
  crop_areas <- colMeans(crop_areas)
  
  scaled_values <- crop_areas * sagbi_multipliers[names(crop_areas)]
  
  sum_sagbi<-sagbi_value*sum(scaled_values)
 ...
 # Initialize an empty vector to store sagbi
land_cover_pmp$rchSuit <- rep(NA, nrow(land_cover_pmp))
 # Iterate over each cell in pu_multi to calculate land cost
 for (i in 1:nrow(land_cover_pmp)) {
   cell <- land_cover_pmp[i, "geometry"]
   land_cover_pmp$rchSuit[i] <- calculate_rchSuit(cell)
 }
#
# Convert sagbi to a raster based on the sagbi field and the spatial extent and resolution of birds_layer
sagbi_raster <- rasterize(land_cover_pmp, birds_layer, field = "rchSuit")

# Define the file path where you want to save the raster
output_path <- "data/sagbi_dust_pmp.tif"
...
#net return over operating costs $/acre 
...
calculate_land_cost <- function(cell, land_cover_data, multipliers) {
  ...
  is_within <- sapply(st_intersects(land_cover_data, cell), function(x) length(x) > 0)
    ...
    land_cost <- sum(land_values)*23722
    return(land_cost)
...
pu_multi$land_cost <- rep(NA, nrow(pu_multi))

# Iterate over each cell in pu_multi to calculate land cost
for (i in 1:nrow(pu_multi)) {
  cell <- pu_multi[i, "geometry"]
  pu_multi$land_cost[i] <- calculate_land_cost(cell, land_cover_pmp,  multipliers)
}
...
st_write(pu_multi, "data/pu_multi/pu_multi_cropareas.shp")
...
# Initialize an empty vector to store land costs
pu_multi$cropareas <- rep(NA, nrow(pu_multi))

# Iterate over each cell in pu_multi to calculate land cost
for (i in 1:nrow(pu_multi)) {
  cell <- pu_multi[i, "geometry"]
  pu_multi$cropareas[i] <- calculate_cropareas(cell, land_cover_pmp)
}
...
pu_multi$water_costk<-pu_multi$cropareas*23715*0.3281/2/1000*pu_multi$cost#convert price $1000/kAF to $ by multiply 10cm depth and 23722 acres per cell
pu_multi$flooded_area<-pu_multi$cropareas*23715/2/1000 #in thousand acres
pu_multi$carbon<-pu_multi$cropars*23715*0.4*0.58/2
...
calculate_carbSeq <- function(cell, land_cover_data) {
  ...
  # Extract the values from land_cover_pmp_dust for the corresponding hexagon cell
  # Multiply each column by corresponding multiplier
  carbSeq <- crop_areas*0.58*23722*0.4/2 # 0.58t/hactare/year convert square acres to hactare
  
  # Calculate the mean of each row
  carbSeq_values <- sum(colMeans(carbSeq))
...
# Define the file path where you want to save the raster
output_path <- "data/carbSeq_dust2.tif"
